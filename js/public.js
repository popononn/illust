	...
	function resetRef(svn){
		...
		ref.on('child_added', function(snapshot) { // ... 3
			...
			$('<li class="btn gl">').html('<a href="' +msg.name+ '" rel="lightbox[plants]" title="説明文 '+msg.month+'\/'+msg.date+' '+msg.hours+':'+msg.minutes+' #1"><img class="smallimage" src="' +msg.body+ '" alt="代替テキスト" /></a>' ).prependTo('#wrap');
